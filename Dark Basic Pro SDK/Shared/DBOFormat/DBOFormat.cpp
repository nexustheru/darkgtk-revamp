	...
	if ( m_pEffect )
	{
		...
		for ( DWORD t=0; t<m_dwTextureCount; t++ )
		{
			...
			if ( t<pMesh->dwTextureCount )
				m_pEffect->SetTexture( m_pEffect->GetParameter( NULL, iParam ), pMesh->pTextures [ t ].pTexturesRef );
		}
...
bool cSpecialEffect::AssignValueHookCore ( LPSTR pName, D3DXHANDLE hParam, DWORD dwClass, bool bRemove )
{
	...
	// WickedX - 100114 r114 - Given new life - Rollback from r86 to fix stencil shadows in DarkGDK
...
bool cSpecialEffect::ParseEffect ( bool bUseEffectXFile, bool bUseEffectTextures )
{
	...
	for( UINT iParam = 0; iParam < m_EffectDesc.Parameters; iParam++ )
	{
		...
		/* code for volume, cube and procedural texture creation (need imageDLL additions)
		if( pstrName != NULL )
		{
		}
...
void cSpecialEffect::ApplyEffect ( sMesh* pMesh )
{
	...
	D3DXMatrixTranspose( &g_EffectConstant.matWorldViewProjT, &g_EffectConstant.matWorldViewProj );
    ...
    if( m_pEffect != NULL )
    {
		...
		// WickedX - 100114 r114 - Given new life - Rollback from r86 to fix stencil shadows in DarkGDK
		// mesh specific values into shader - This gave its life on 130411 to protect size of DBO (in favour of clip plane)
...
DARKSDK_DLL bool CalcObjectWorld ( sObject* pObject )
{
	...
	if ( pObject->position.iGluedToObj )
	{
		...
		sObject* pChildObject = g_ObjectList [ pObject->position.iGluedToObj ];
		...
		pGluedToFramePtr = g_ObjectList [ pObject->position.iGluedToObj ]->ppFrameList[ iFrame ];
		...
		CalculateAbsoluteWorldMatrix ( pChildObject, pGluedToFramePtr, pGluedToFramePtr->pMesh );
	}
...
//  leefix - 140604 - moved further up - prior to code that updated the VBs
...
//	UpdateFrame ( pObject->pFrame, &matrix );
	...
	// leefix - 250604 - u54 - all frames should be flagged for recalc of vectors (need data prior to sync!)
...
DARKSDK_DLL void UpdateLocalMeshWithDXMesh ( sMesh* pMesh, LPD3DXMESH pDXMesh )
{
	...
	if ( pDXMesh->GetFVF()==0 )
	{
		...
		SetupMeshDeclarationData ( pMesh, Declaration, pDXMesh->GetNumBytesPerVertex(), dwVertexCount, dwIndexCount );
	}
	else
	{
...
DARKSDK_DLL LPD3DXMESH ComputeTangentBasisEx ( LPD3DXMESH gMasterMesh, bool bMakeNormals, bool bMakeTangents, bool bMakeBinormals, bool bFixTangents, bool bCylTexGen, bool bWeightNormalsByFace )
{
	...
	gMasterMesh->CloneMeshFVF ( D3DXMESH_SYSTEMMEM, D3DFVF_XYZ | D3DFVF_NORMAL | D3DFVF_TEX1, m_pD3D, &pInputDataMesh );
	if ( pInputDataMesh==NULL ) return gMasterMesh;
	// leefix - 050906 - this destroys all the unique data in pMesh with the above - use pNewMesh as a temp mesh to generate the tangents, then copy back into master mesh
	//	// Switch to use new temporary mesh
	//	if ( gMesh ) gMesh->Release();
	//	gMesh = pNewMesh;
	...
	if ( dwNewVertexCount==numVertices && dwNewFaceCount==numTriangles )
	{
		...
		gMasterMesh->CloneMesh( D3DXMESH_SYSTEMMEM, Declaration, m_pD3D, &pOutputMesh );
	}
	else
	{
		// create new mesh resized for new output data
		D3DXCreateMesh ( dwNewFaceCount, dwNewVertexCount, D3DXMESH_SYSTEMMEM, Declaration, m_pD3D, &pOutputMesh);
	}
...
DARKSDK_DLL void GenerateNormalsForMesh ( sMesh* pMesh )
{
	...
	if( !bHasNormals ) 
	{
		...
		HRESULT hr = pDXMesh->CloneMesh( D3DXMESH_SYSTEMMEM, Declaration, m_pD3D, &pTempMesh );
		if( SUCCEEDED( hr ) )
		{
			SAFE_RELEASE( pDXMesh );
			pDXMesh = pTempMesh;
...
DARKSDK_DLL void UpdateObjectAnimation ( sObject* pObject )
{
	...
	// U73 - 240309 - replaces (else) that was mistakenly here, excluding all animations
...
DARKSDK_DLL bool CalculateAllBounds ( sObject* pObject, bool bUpdateOnly )
{
...
DARKSDK_DLL bool SetupMeshDeclarationData ( sMesh* pMesh, CONST D3DVERTEXELEMENT9* pDeclaration, DWORD dwVertexSize, DWORD dwVertexCount, DWORD dwIndexCount )
{
	...
	if ( FAILED ( m_pD3D->CreateVertexDeclaration ( pDeclaration, &pMesh->pVertexDec ) ) )
		return false;
