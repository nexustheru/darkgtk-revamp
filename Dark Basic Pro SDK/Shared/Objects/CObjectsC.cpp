...
DARKSDK_DLL void SetTransparency ( int iID, int iTransparency )
{
	...
	// 7 - U71 - very early draw phase no alpha
...
DARKSDK_DLL void SetObjectFilter(int iID, int iMagState, int iMinState)
{
	int iMesh;
	sObject* pObject;
	sMesh* pMesh;
	...
	pObject = g_ObjectList[iID];

	for (iMesh=0; iMesh<pObject->iMeshCount; iMesh++ ) {
		pMesh = pObject->ppMeshList[iMesh];
		pTexture = &pMesh->pTextures[0];
		if (pTexture) {
			if (iMagState)
				pTexture->dwMagState = iMagState;
			if (iMinState)
				pTexture->dwMinState = iMinState;
		}
	}
}

DARKSDK_DLL void SetObjectFilterStage(int iID, int iStage, int iMagState, int iMinState)
{
	int iMesh;
	sObject* pObject;
	sMesh* pMesh;
	...
	for (iMesh=0; iMesh<pObject->iMeshCount; iMesh++ ) {
		pMesh = pObject->ppMeshList[iMesh];
		pTexture = &pMesh->pTextures[iStage];
		if (pTexture) {
			if (iMagState)
				pTexture->dwMagState = iMagState;
			if (iMinState)
...
DARKSDK_DLL void SetLimbFilter(int iID, int iLimbID, int iMagState, int iMinState)
{
	sObject* pObject;
	sMesh* pMesh;
	sTexture* pTexture;
	...
	pMesh = pObject->ppFrameList[iLimbID]->pMesh;
...
DARKSDK_DLL void SetLimbFilterStage(int iID, int iLimbID, int iStage, int iMagState, int iMinState)
{
	sObject* pObject;
	sMesh* pMesh;
	sTexture* pTexture;
...
DARKSDK_DLL void SetDiffuseMaterialEx ( int iID, DWORD dwRGB, int iMaterialOrVertexData )
{
	...
	if ( pActualObject )
	{
		...
		if ( pActualObject->iMeshCount>0 )
		{
			...
			{
				// create custom slot
				pObject->dwCustomSize = 4;
				pObject->pCustomData = (LPVOID)new DWORD;
				*(DWORD*)pObject->pCustomData = 0;
			}

			// set diffuse colour
			*(DWORD*)pObject->pCustomData = dwRGB;
		}
	}
	else
	{
...
DARKSDK_DLL void CloneShared ( int iDestinationID, int iSourceID, int iCloneSharedData )
{
	...
	{
		sMesh* pMesh = pObject->ppMeshList [ iMesh ];
		sMesh* pNewMesh = pNewObject->ppMeshList [ iMesh ];
	...
	if ( g_ObjectList [ iDestinationID ]->ppMeshList )
	{
		...
		{
			// get mesh ptr
			sMesh* pMesh = g_ObjectList [ iDestinationID ]->ppMeshList [ iMesh ];
			sMesh* pOrigMesh = pObject->ppMeshList [ iMesh ];
			if ( pMesh && pOrigMesh )
			{
				...
				// regular mesh clone texture (multimaterial still uses texture)
				CloneInternalTextures ( pMesh, pOrigMesh );
			}
		}
	}

	// mike - 011005 - position data of cloned object must match source
...
// WickedX - 270814 r114 - Added internal function MakeGroundPlane(int iID, float fWidth, float fDepth)
DARKSDK_DLL void MakeGroundPlane(int iID, float fWidth, float fDepth)
{
	DWORD dwVertexCount;
	DWORD dwIndexCount;
...
// leefix - 190503 - make compatibl with P3 and earlier
//	// apply setting to all meshes
//	sObject* pObject = g_ObjectList [ iID ];
//	for ( int iMesh = 0; iMesh < pObject->iMeshCount; iMesh++ )
//		Hide ( pObject->ppMeshList [ iMesh ] );
...
{
	// check the object exists
	// leefix - 210806 - allow instanced objects to acquire camera orientation
	if ( !ConfirmObjectInstance ( iID ) )
		return;
...
DARKSDK_DLL void SetAlphaFactor ( int iID, float fPercentage )
{
	...
	{
		// mike - 300905 - disable original code
...
DARKSDK_DLL void DeleteEffect ( int iEffectID )
{
	...
	// mike - 120307 - check all other objects that reference this effect, if they reference
	//               - it then remove the link so it wont cause a crash later on
...
DARKSDK_DLL void SetEffectTechnique	( int iEffectID, SDK_LPSTR pTechniqueName )
{
	...
	if ( pEffectPtr )
	{
		...
		if ( hTechnique )
			// MIKE - 021003 - disable for now
			//if ( pEffectPtr->ValidateTechnique ( hTechnique )==D3D_OK )
...
DARKSDK_DLL void CloneMeshToNewFormat ( int iID, DWORD dwFVF )
{
	// check the object exists
	if ( !ConfirmObject ( iID ) )
		return;
...
{
	// check the object exists
	// mike - 190107 - allow instanced objects
...
DARKSDK_DLL void ShowBoundsEx ( int iID, int iBoxOnly )
{
	...
	// mike - 290305 - make sure object is updated in buffer else it won't draw
...
DARKSDK_DLL void PickScreen2D23D ( int iX, int iY, float fDistance )
{
	...
	// calculate line from camera to distant point through projected XY
	D3DXVECTOR3 vecFrom = m_Camera_Ptr->vecPosition;
...
// WickedX - 281014 r114 - pick screen on xz-plane
DARKSDK_DLL D3DXVECTOR3 Get3DPointOnXZPlane(float fYPos, int iXPos, int iYPos)
{
	int iObjectID;
	float fDistance;
	float X, Y, Z;
	...
	tagCameraData* m_Camera_Ptr = (tagCameraData*)g_Camera3D_GetInternalData ( g_pGlob->dwCurrentSetCameraID );
	D3DXVECTOR3 vecCamPos = m_Camera_Ptr->vecPosition;

	iObjectID = g_DBPROPickResult.iObjectID;
	fDistance = g_DBPROPickResult.fPickDistance;
	vec1 = g_DBPROPickResult.vecPickVector;

	PickScreen2D23D(iXPos, iYPos, vecCamPos.y);
	vec2 = g_DBPROPickResult.vecPickVector;
	X = vec2.x;
	Y = vec2.y;
	Z = vec2.z;

	X = X*-vecCamPos.y/Y+vecCamPos.x;
	Z = Z*-vecCamPos.y/Y+vecCamPos.z;

	g_DBPROPickResult.iObjectID = iObjectID;
	g_DBPROPickResult.fPickDistance = fDistance;
	g_DBPROPickResult.vecPickVector = vec1;
...
// WickedX - 260814 r114 - Added internal function ProjectPoint(float xPos, float yPos, float zPos, int &x, int &y)
DARKSDK_DLL void ProjectPoint(float xPos, float yPos, float zPos, int &x, int &y)
{
	float fVPWidth;
	float fVPHeight;
	float fRealX;
	float fRealY;
	D3DXVECTOR3 vecBob;
	tagCameraData* m_Camera_Ptr;

	vecBob = D3DXVECTOR3(xPos, yPos, zPos);

	// camera ptr
	m_Camera_Ptr = (tagCameraData*)g_Camera3D_GetInternalData ( g_pGlob->dwCurrentSetCameraID );

	// get current camera transformation matrices
	D3DXMATRIX matTransform = m_Camera_Ptr->matView * m_Camera_Ptr->matProjection;

	// Transform coords from world-space to screen-space
	...
	// adjust coordinates using viewport of the target camera
	fVPWidth = m_Camera_Ptr->viewPort3D.Width;
	fVPHeight = m_Camera_Ptr->viewPort3D.Height;
	fRealX = ( fVPWidth / g_pGlob->iScreenWidth ) * (x);
	fRealY = ( fVPHeight / g_pGlob->iScreenHeight ) * (y);
...
// WickedX - 111014 - Added function - returns maximum anisotropic filter available
DARKSDK_DLL int GetMaxAnisotropicLevel ( void )
{
	return (int)g_pGlob->dwMaxAnisotropyLevel;
}

// WickedX - 111014 - Added function - set anisotropic filter level
DARKSDK_DLL void SetAnisotropicLevel(int iAnisotropicLevel)
{
	g_pGlob->dwAnisotropyLevel = (DWORD)iAnisotropicLevel;
}
...
DARKSDK_DLL void ExcludeOn ( int iID )
{
	...
	// WickedX - 121014 r114 - see no logical reason for this line
...
// WickedX - 290914 r114 - Added DarkGDK wrapper for dbSetObjectMask()
void dbSetObjectMask ( int iID, int iMASK )
{
	SetMask ( iID, iMASK );
}
...
// WickedX - 270814 r114 - Added DarkGDK wrapper for dbMakeObjectPlane(iID, fWidth, fHeight, iFlag)
void dbMakeObjectPlane ( int iID, float fWidth, float fHeight, int iFlag )
{
	MakePlane ( iID, fWidth, fHeight, iFlag);
}

// WickedX - 200114 r114 - Added DarkGDK wrapper for dbMakeObjectGroundPlane(int iID, float fWidth, float fDepth)
void dbMakeObjectGroundPlane(int iID, float fWidth, float fDepth)
{
	MakeGroundPlane(iID, fWidth, fDepth);
}
...
// WickedX - 240814 r114 - Added wrapper for dbAddMemblockToObject(iMemblock, iID)
void dbAddMemblockToObject ( int iMemblock, int iID )
{
	AddMemblockToObject(iMemblock, iID);
}

// WickedX - 240814 r114 - Added wrapper for dbGetMemblockFromObject(iMemblock, iID)
void dbGetMemblockFromObject ( int iMemblock, int iID )
{
	GetMemblockFromObject(iMemblock, iID);
}

// WickedX - 240814 r114 - Added wrapper for DeleteMemblockFromObject(iID)
void dbDeleteMemblockFromObject ( int iID )
{
	DeleteMemblockFromObject(iID);
}
...
// WickedX - 281014 r114 - function wrapper for dbGet3dPointOnXZPlane()
D3DXVECTOR3 dbGet3dPointOnXZPlane ( float fYPos, int iXPos, int iYPos )
{
	return Get3DPointOnXZPlane(fYPos, iXPos, iYPos);
}
...
// WickedX - 240814 r114 - Added wrapper for function dbProjectPoint(float xPos, float yPos, float zPos, int &x, int &y)
void dbProjectPoint ( float xPos, float yPos, float zPos, int &x, int &y )
{
	ProjectPoint(xPos, yPos, zPos, x, y);
}
...
// WickedX - 240814 r114 - Added function wrapper for dbPositionObject(int id, const D3DXVECTOR3 position)
void dbPositionObject ( int iID, D3DXVECTOR3 pos )
{
	Position(iID, pos);
}
...
// WickedX - 250814 r114 - Added function wrapper for D3DXVECTOR3 dbObjectPosition (int iID)
D3DXVECTOR3 dbObjectPosition ( int iID )
{
	D3DXVECTOR3 Result;
	
	Result = ObjectPosition (iID);
	return Result;
}
...
// WickedX - 200114 r114 - Added DarkGDK wrapper for dbMakeObjectPlain(iID, fWidth, fHeight, iFlag)
void dbMakeObjectPlain ( int iID, float fWidth, float fHeight, int iFlag ) { return dbMakeObjectPlane (iID, fWidth, fHeight, iFlag); }
...
// WickedX - 111014 - Added function prototype for dbMaxAnisotropicLevel()
int dbMaxAnisotropicLevel ( void ) { return GetMaxAnisotropicLevel(); }
// WickedX - 111014 - Added function prototype for dbSetAnisotropicLevel()
void dbSetAnisotropicLevel(int iAnisotropicLevel) { SetAnisotropicLevel(iAnisotropicLevel); }
