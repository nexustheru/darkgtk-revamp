...
// WickedX - 240814 r114 - Added internal function Position(int id, const D3DXVECTOR3 position)
DARKSDK void Position ( int iID, D3DXVECTOR3 pos )
{
	// check the object exists
	if ( !ConfirmObjectInstance ( iID ) )
		return;
	...
	sObject* pObject = g_ObjectList [ iID ];
	pObject->position.vecLastPosition = pObject->position.vecPosition;
	pObject->position.vecPosition = pos;
...
// WickedX - 250814 r114 - Added internal function D3DXVECTOR3 ObjectPosition (int iID)
DARKSDK D3DXVECTOR3 ObjectPosition (int iID)
{
	D3DXVECTOR3 Result;
	...
	sObject* pObject = g_ObjectList [ iID ];
		...
		if ( pObject->position.bGlued == true )
		{
			...
			Result = D3DXVECTOR3(pObject->position.matWorld._41, pObject->position.matWorld._42, pObject->position.matWorld._43);
			return Result;
		}
	#endif

	Result = D3DXVECTOR3(pObject->position.vecPosition.x, pObject->position.vecPosition.y, pObject->position.vecPosition.z);
	return Result;
}
