...
// WickedX - 050314 r114 - Added internal function for dbFindFont
DARKSDK BOOL FindFont(LPSTR szFont)
{
	BOOL Result;
	int i;
	DWORD ci;
	...
	for (i=0; i<g_pGlob->checklistqty; ++i) {
		if (!strcmp(g_pGlob->checklist[i].string, szFont)) {
			Result = true;
			break;
		}
	}
	...
	g_pGlob->checklistqty = 0;
	for(ci=0; ci<g_pGlob->dwChecklistArraySize; ci++)
	{
...
// WickedX - 050314 r114 - Added internal function for dbLoadFont
DARKSDK BOOL LoadFont(LPSTR szFont)
{
	BOOL Result;

	Result = AddFontResource(szFont);
...
// WickedX - 050314 r114 - Added internal function for dbUnloadFont
DARKSDK BOOL UnloadFont(LPSTR szFont)
{
	BOOL Result;

	Result = RemoveFontResource(szFont);
...
// WickedX - 050314 r114 - Added DarkGDK wrapper for dbFindFont
BOOL dbFindFont ( LPSTR szFont )
{
	return FindFont( szFont );
}
// WickedX - 050314 r114 - Added DarkGDK wrapper for dbLoadFont
BOOL dbLoadFont ( LPSTR szFont )
{
	return LoadFont( szFont );
}
// WickedX - 050314 r114 - Added DarkGDK wrapper for dbUnloadFont
BOOL dbUnloadFont ( LPSTR szFont )
{
	return UnloadFont ( szFont );
}
