...
DARKSDK void EncryptDecrypt( DWORD dwStringAddress, bool bEncryptIfTrue )
{
	...
	if(DoesFileExist(pFilename))
	{
		...
		Encryptor.SetUniqueKey(g_Glob.dwEncryptionUniqueKey);
...
DARKSDK void Decrypt( DWORD dwStringAddress )
{
	...
	if ( strnicmp ( (LPSTR)dwStringAddress, g_Glob.pEXEUnpackDirectory, strlen(g_Glob.pEXEUnpackDirectory) )==NULL )
		EncryptDecrypt ( dwStringAddress, false );
}
...
DARKSDK void Encrypt( DWORD dwStringAddress )
{
	// lee - 230306 - u6b4 - only encrypt/decrypt if from DBPDATA temp folder (should not touch local files!)
	if ( strnicmp ( (LPSTR)dwStringAddress, g_Glob.pEXEUnpackDirectory, strlen(g_Glob.pEXEUnpackDirectory) )==NULL )
		EncryptDecrypt ( dwStringAddress, true );
}
...
DARKSDK DWORD InitDisplayEx(DWORD dwDisplayType, DWORD dwWidth, DWORD dwHeight, DWORD dwDepth, HINSTANCE hInstance, LPSTR pApplicationName, HWND pParentHWND, DWORD dwInExStyle, DWORD dwInStyle)
{
	...
	// Process any messages prior to program start (also for beginscene call)
...
DARKSDK DWORD AbsFF(float fValue)
{
...
DARKSDK DWORD CosFF(float fAngle)
{
	float result = (float)cos(fAngle*gDegToRad);
	return *((DWORD*)&result);
}

DARKSDK DWORD SinFF(float fAngle)
{
	float result = (float)sin(fAngle*gDegToRad);
	return *((DWORD*)&result);
}

DARKSDK DWORD TanFF(float fAngle)
{
...
// WickedX - 160514 r114 - Optimized with inline assembly using the FPU
// This will now return a float as a float not DWORD
// First necessary break in DBPro's dll support. Can optimize more
DARKSDK float SqrtFF(float fValue)
{
	__asm {
		fld fValue
		fsqrt
...
*/
// WickedX - 180614 r114- Added internal function for dbRnd - float variant
DARKSDK float RndFF(float r)
{
	float result;
	result = 0;
	
	if(r>0) 
		result = static_cast <float> (rand()) / static_cast <float> (RAND_MAX/r);
	return result;
}
