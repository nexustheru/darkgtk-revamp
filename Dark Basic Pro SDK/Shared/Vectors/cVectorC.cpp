...
// WickedX - 261014 r114 - Set Projection Matrix
DARKSDK void SetProjectionMatrix ( int iID )
{
	// pointer to matrix data
	D3DXMATRIX* pMatrix = NULL;

	// check the operation can be carried out on this type
	if ( !CheckTypeIsValid ( iID, TYPE_MATRIX ) )
	{
		RunTimeError ( RUNTIMEERROR_VECTORNOTEXIST );
		return;
	...
	if ( g_Camera3D_GetInternalData )
	{
		// get camera pointer
		tagCameraData* m_Camera_Ptr = (tagCameraData*)g_Camera3D_GetInternalData ( g_pGlob->dwCurrentSetCameraID );
		m_Camera_Ptr->matProjection = *pMatrix;
	}
}
...
// WickedX - 221014 r114 - Reversed order of parameters
DARKSDK void CopyVector4 ( int iDestination, int iSource )
{
	...
	// get the destination vector
	// WickedX - 221014 r114 - Should get iDestination not iSource
	//if ( ! ( pVecDestination = ( D3DXVECTOR4* ) m_DataManager.GetData ( iSource ) ) )
	if ( ! ( pVecDestination = ( D3DXVECTOR4* ) m_DataManager.GetData ( iDestination ) ) )
	{
	...
	pVecDestination->y = pVecSource->y;
	pVecDestination->z = pVecSource->z;
	pVecDestination->w = pVecSource->w;
}
...
// WickedX - 261014 r114 - Set Matrix Element
DARKSDK void SetMatrixElement ( int iMatrix, int iElement, float fValue )
{
	D3DXMATRIX Matrix;
	D3DXMATRIX* pMatrix;
	...
	pMatrix = ( D3DXMATRIX* ) m_DataManager.GetData ( iMatrix );
	Matrix = GetMatrix(iMatrix);
	if (iElement>=0 && iElement<=15)
		Matrix[iElement] = fValue;
	*pMatrix = Matrix;
}
...
// WickedX - 221014 r114 - Reversed order of parameters - Did same in DarkSDK3DMathe.h
void dbCopyVector2 ( int iDestination, int iSource )
{
	CopyVector2 ( iDestination, iSource );
}
...
// WickedX - 221014 r114 - Reversed order of parameters - did same in DarkSDK3DMathe.h
void dbCopyVector3 ( int iDestination, int iSource )
{
	CopyVector3 ( iDestination, iSource );
}
...
// WickedX - 221014 r114 - Reversed order of parameters - did same in DarkSDK3DMathe.h
void dbCopyVector4 ( int iDestination, int iSource )
{
	CopyVector4 ( iDestination, iSource );
}
...
// WickedX - 261014 r114 - function wrapper for dbSetProjectionMatrix4()
void dbSetProjectionMatrix4 ( int iID )
{
	SetProjectionMatrix ( iID );
}
...
// WickedX - 261014 r114 - function wrapper for dbSetMatrixElement()
void dbSetMatrixElement ( int iMatrix, int iElement, float fValue )
{
	SetMatrixElement ( iMatrix, iElement, fValue );
}
