...
DARKSDK void ConstructorD3D ( HINSTANCE hSetup )
{
	...
	if ( !hSetup )
	{
		...
		if ( !hSetup )
			Error ( "Cannot load setup library for image library" );
	}
	#endif
...
DARKSDK int LoadInternal ( char* szFilename, int iDivideTextureSize )
{
	...
	if ( !FindInternalImage ( szFilename, &iImageID ) )
	{
		...
		if ( Load ( szFilename, iImageID, 0, true, iDivideTextureSize ) )
		{
			// new image returned
			return iImageID;
		}
...
DARKSDK void SetMipmapMode ( bool bMode )
{
	...
	if ( !bMode )
	{
		m_pD3D->SetSamplerState ( 0, D3DSAMP_MAGFILTER, D3DTEXF_NONE );
		m_pD3D->SetSamplerState ( 0, D3DSAMP_MINFILTER, D3DTEXF_NONE );
...
DARKSDK bool Save ( char* szFilename, int iID, int iCompressionMode )
{
	...
	// leefix - 031003 - determine ANY size of surface
	D3DSURFACE_DESC ddsd;
	HRESULT hRes = pTexSurface->GetDesc(&ddsd);
	RECT rc = { 0, 0, (int)((float)ddsd.Width*m_ptr->fTexUMax), (int)((float)ddsd.Height*m_ptr->fTexVMax) };

	// leeadd - 210704 - If DDS, use automatic compression
	...
	if ( DestFormat==D3DXIFF_DDS )
	{
		...
		m_pD3D->CreateTexture ( ddsd.Width, ddsd.Height, 1, 0, dwD3DSurfaceFormat, D3DPOOL_MANAGED, &pTextureDDS, NULL );
		if ( pTextureDDS )
		{
			...
			if ( pDDSSurface )
			{
				if ( pTexSurface )
				{
					D3DXLoadSurfaceFromSurface ( pDDSSurface, NULL, NULL, pTexSurface, NULL, NULL, D3DX_DEFAULT, 0 );
				}
			}
...
int GetImageFileExist ( LPSTR pFilename )
{
	...
	if ( g_bImageBlockActive && g_iImageBlockMode==1 )
	{
		...
		DWORD dwFileInMemorySize = 0;
		GetFileInMemory ( pFilename, &pFileInMemoryData, &dwFileInMemorySize, pFinalRelPathAndFile );
...
// WickedX - 200114 r114 - Added DarkGDK wrappers for ImageBlock Functions
void dbOpenImageBlock ( char* szFilename, int iMode )
{
	OpenImageBlock ( szFilename, iMode );
}
void dbExcludeFromImageBlock ( char* szExcludePath )
{
	ExcludeFromImageBlock ( szExcludePath );
}
void dbCloseImageBlock ( void )
{
	CloseImageBlock ( );
}
void dbPerformChecklistForImageBlockFiles ( void )
{
	PerformChecklistForImageBlockFiles ( );
}
int dbGetImageFileExist ( LPSTR pFilename )
{
	return GetImageFileExist ( pFilename );
}
//
#endif
