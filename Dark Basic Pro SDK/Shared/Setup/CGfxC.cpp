...
DARKSDK void ShowDemoLogo(void)
{
	...
	if(g_pDemoTexture)
	{
		// get dynamic states
		DWORD dwCurrentFog;
		m_pD3DDevice->GetRenderState ( D3DRS_FOGENABLE, &dwCurrentFog );
		...
		VERTEX2D vertexData [ ] =
		{
			{  g_pGlob->iScreenWidth - WIDTH,			g_pGlob->iScreenHeight - HEIGHT,			0.5f,	1.0f,	D3DCOLOR_ARGB ( 127, 255, 255, 255 ),		0.0f,	0.0f },
			{  g_pGlob->iScreenWidth - WIDTH + WIDTH,	g_pGlob->iScreenHeight - HEIGHT,			0.5f,	1.0f,	D3DCOLOR_ARGB ( 127, 255, 255, 255 ),		1.0f,	0.0f },
			{  g_pGlob->iScreenWidth - WIDTH,			g_pGlob->iScreenHeight - HEIGHT + HEIGHT,	0.5f,	1.0f,	D3DCOLOR_ARGB ( 127, 255, 255, 255 ),		0.0f,	1.0f },
			{  g_pGlob->iScreenWidth - WIDTH + WIDTH,	g_pGlob->iScreenHeight - HEIGHT + HEIGHT,	0.5f,	1.0f,	D3DCOLOR_ARGB ( 127, 255, 255, 255 ),		1.0f,	1.0f }
		};

		// set vertex shader and texture
		m_pD3DDevice->SetVertexShader ( NULL );
		m_pD3DDevice->SetFVF ( D3DFVF_VERTEX2D );
		m_pD3DDevice->SetTexture ( 0, g_pDemoTexture );
		...
		m_pD3DDevice->SetTextureStageState ( 0, D3DTSS_ALPHAOP,		D3DTOP_MODULATE);//SELECTARG1 );
		m_pD3DDevice->SetTextureStageState ( 0, D3DTSS_ALPHAARG1,	D3DTA_TEXTURE );
		m_pD3DDevice->SetTextureStageState ( 0, D3DTSS_ALPHAARG2,	D3DTA_DIFFUSE );
		m_pD3DDevice->SetTextureStageState ( 1, D3DTSS_COLOROP,		D3DTOP_DISABLE );

		// draw watermark
		m_pD3DDevice->DrawPrimitiveUP ( D3DPT_TRIANGLESTRIP, 2, vertexData, sizeof ( VERTEX2D ) );

		// restore all important states to standard
		m_pD3DDevice->SetRenderState ( D3DRS_FILLMODE,				D3DFILL_SOLID );
		...
		// states that are dynamically restored
		m_pD3DDevice->SetRenderState ( D3DRS_FOGENABLE,				dwCurrentFog );
	}
	#endif
}
...
DARKSDK bool Setup ( void )
{
	...
	if(m_pD3DDevice==NULL)
	{
		...
		// WickedX - 081014 - check maxanisotropy and set globstruct values
		// best card can give
		D3DCAPS9 pCaps;
		m_pD3DDevice->GetDeviceCaps(&pCaps);
		g_pGlob->dwMaxAnisotropyLevel = pCaps.MaxAnisotropy;
		g_pGlob->dwAnisotropyLevel = g_pGlob->dwMaxAnisotropyLevel;
												...
												) ) )
	{
		...
		if ( FAILED ( hr = m_pD3D->CreateDevice
										(
											...
											hWnd,									// handle to window
											D3DCREATE_SOFTWARE_VERTEXPROCESSING,	// software processing
											d3dpp,									// display info
											&m_pD3DDevice							// pointer to device
										) ) )
		{
...
DARKSDK void SetGraphicsCard ( DWORD dwCardname )
{
	...
	/*
	// Find match with card name and set the adapter if successful
	bool bChangeDevice=false;
	for ( int iTemp = 0; iTemp < m_iAdapterCount; iTemp++ )
	{
		if(stricmp(m_pInfo [ iTemp ].szName, (LPSTR)dwCardname)==NULL)
		{
			// Set New Adapter to use
			m_iAdapterUsed = iTemp;
			bChangeDevice=true;
		}
	}
	// If Device to be changed, everything must be freed and new adapter set
	if(bChangeDevice==true)
	{
		// Free ALL
		// Create new ALL
	}
...
// WickedX - 290414 r114 - Added variants to dbSetDisplayMode(Width, Height, Depth) for those familiar with DBPro
//////////////////////////////////////////////////////////////////////////////////
bool dbSetDisplayMode ( int iWidth, int iHeight, int iDepth, int iVSyncOn )
{
	SetDisplayModeVSYNC ( iWidth, iHeight, iDepth, iVSyncOn );
	return true;
}

bool dbSetDisplayMode ( int iWidth, int iHeight, int iDepth, int iVSyncOn, int iMultisamplingFactor, int iMultimonitorMode )
{
	SetDisplayModeANTIALIAS ( iWidth, iHeight, iDepth, iVSyncOn, iMultisamplingFactor, iMultimonitorMode );
	return true;
}
