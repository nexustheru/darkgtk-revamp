...
DARKSDK void SetupKeyboard ( void )
{
	...
	// request foregound, non exclusive and disable windows key
...
DARKSDK void SetupMouse ( void )
{
	...
	if ( FAILED ( hr = m_lpDIMouse->SetCooperativeLevel ( g_phWnd, DISCL_NONEXCLUSIVE | DISCL_FOREGROUND ) ) )
	{
		Destructor ( );
...
DARKSDK void PositionMouse( int iX, int iY )
{
	...
	if(iYMargin!=0)
	{
		...
		//iY += windowrc.top - iXMargin + iYMargin + 1;
