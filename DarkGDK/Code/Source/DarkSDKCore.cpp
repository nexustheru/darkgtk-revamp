...
float 		SqrtFF					( float fValue ); // WickedX - 160514 r114 - Changed DWORD to float
DWORD 		ExpFF					( float fExp );
void 		Randomize				( int iSeed );
int 		RndLL					( int r );
float		RndFF					( float r ); // WickedX - 180614 r114 - random float variant
...
int 		TimerL					( void );
// WickedX - 170514 r114 - Added - dbPerformanceTimer and PerformanceFrequency
LONGLONG	PerformanceTimer		( void );
LONGLONG	PerformanceFrequency	( void );
...
// WickedX - 240914 r114 - Added - AddToRenderList and RemoveFromRenderList
void		AddToRenderList			( LPVOID pFunction, int iPriority );
void		RemoveFromRenderList	( LPVOID pFunction );
...
// WickedX - 160514 r114 - Optimized with inline assembly using FPU
// This will now return a float as a float not DWORD
float dbSqrt ( float fValue )
{
	float result;

	result = SqrtFF ( fValue );
	return result;
}
...
// WickedX - 180614 r114 - Added DarkGDK wrapper for dbRnd - float variant
float dbRnd ( float dbr )
{
	return RndFF ( dbr );
}
...
// WickedX - 170514 r114 - Added DarkGDK wrapper for dbPerformanceTimer and dbPerformanceFrequency
LONGLONG dbPerformanceTimer ( void )
{
	return PerformanceTimer ( );
}

LONGLONG dbPerformanceFrequency ( void )
{
...
// WickedX - 240914 r114 - Added - dbAddToRenderList and dbRemoveFromRenderList
void dbAddToRenderList(LPVOID pFunction, int iPriority)
{
	AddToRenderList(pFunction, iPriority);
}

void dbRemoveFromRenderList(LPVOID pFunction)
{
	RemoveFromRenderList(pFunction);
}

int			dbRND					( int dbr )				{ return dbRnd ( dbr ); }
float		dbRND					( float dbr )			{ return dbRnd ( dbr ); }
